...
public class WorldGenMinableCloud extends WorldGenerator {
	...
	public WorldGenMinableCloud(int i, int rad, int den, int amo, int j) {
		minableBlockId = i;
		radius = rad;
		density = den;
		amount = amo;
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for(int o = 0; o < radius*2; o++) for(int l = 0; l < radius*2; l++) for(int f = 0; f < radius*2; f++) {
			if(pregen[o][l][f] && world.getBlockId(o+i, l+j, k+f) == generateIn){
	...
	int minableBlockId, radius, density, amount, generateIn;
